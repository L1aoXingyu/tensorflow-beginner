...
mnist = input_data.read_data_sets(
    '/home/sherlock/Desktop/tensorflow-beginner/lab/data/mnist', one_hot=True)
...
with tf.variable_scope('layer1'):
    ...
    b1 = tf.get_variable('bias_1', [256], initializer=tf.zeros_initializer())

with tf.variable_scope('layer2'):
    w2 = tf.get_variable(
        'weight_2', [256, 128],
        initializer=tf.contrib.layers.xavier_initializer())
    b2 = tf.get_variable('bias_2', [128], initializer=tf.zeros_initializer())

with tf.variable_scope('layer3'):
    w3 = tf.get_variable(
        'weight_3', [128, 10],
        initializer=tf.contrib.layers.xavier_initializer())
    b3 = tf.get_variable('bias_3', [10], initializer=tf.zeros_initializer())
# Step 4: build model
...
logits = tf.matmul(layer2_out, w3) + b3
